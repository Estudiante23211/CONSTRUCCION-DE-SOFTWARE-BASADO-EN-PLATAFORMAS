...
export function validarTipoClienteNombreCreacion(nombre: string): ResultadoValidacion {
...
export function validarTipoClienteNombreUnicoEnListado(
  tipos: readonly { id: number; nombre: string }[],
  nombre: string,
...
export function validarTipoClienteDescripcionCreacion(descripcion: string): ResultadoValidacion {
...
export function validarTipoClienteNombreContenido(nombre: string): ResultadoValidacion {
  const t = nombre.trim();
  const len = longitudValida(t, 5, 200, 'El nombre');
  if (!len.valido) return len;
  if (!soloLetrasYEspacios(nombre)) {
...
export function validarTipoClienteDescripcionContenido(descripcion: string): ResultadoValidacion {
  const t = descripcion.trim();
  const len = longitudValida(t, 10, 200, 'La descripción');
  if (!len.valido) return len;
  if (contieneSQLInjection(t)) return { valido: false, mensaje: MSG_SQL };
