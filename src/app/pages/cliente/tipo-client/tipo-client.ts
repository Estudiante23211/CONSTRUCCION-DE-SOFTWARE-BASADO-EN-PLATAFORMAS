...
export type TipoClienteModel = ClienteCatalogoItem;
...
export class TipoClient implements OnInit {
  private readonly recurso = 'tipos-cliente' as const;
  ...
  tipoDetalle: TipoClienteModel | null = null;
  ...
  get tiposFiltradosPaginados(): TipoClienteModel[] {
  ...
  abrirNuevo(): void {
    ...
    setTimeout(() => this.showModal('modalTipoClienteForm'), 0);
  }
  ...
  abrirDetalle(tipo: TipoClienteModel): void {
    this.tipoDetalle = tipo;
    this.saveError = '';
    setTimeout(() => this.showModal('modalTipoClienteDetalle'), 0);
  }

  cerrarModalFormulario(): void {
    this.saveError = '';
    this.hideModal('modalTipoClienteForm');
  }

  cerrarModalDetalle(): void {
    this.hideModal('modalTipoClienteDetalle');
  ...
  guardar(): void {
    ...
    const vN = validarTipoClienteNombreCreacion(this.form.nombre);
    if (!vN.valido) {
      this.saveError = vN.mensaje;
      return;
    }
    const vU = validarTipoClienteNombreUnicoEnListado(this.tipos, this.form.nombre, null);
    if (!vU.valido) {
      this.saveError = vU.mensaje;
      return;
    }
    const vD = validarTipoClienteDescripcionCreacion(this.form.descripcion);
    if (!vD.valido) {
  ...
  private guardarEdicion(): void {
    ...
    const vNom = validarTipoClienteNombreContenido(f.nombre);
    if (!vNom.valido) {
      this.saveError = vNom.mensaje;
      return;
    }
    const vUni = validarTipoClienteNombreUnicoEnListado(this.tipos, f.nombre, id);
    if (!vUni.valido) {
      this.saveError = vUni.mensaje;
      return;
    }
    const vDesc = validarTipoClienteDescripcionContenido(f.descripcion);
    if (!vDesc.valido) {
