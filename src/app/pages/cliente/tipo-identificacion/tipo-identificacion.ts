...
export type TipoIdentificacionModel = ClienteCatalogoItem;
...
export class TipoIdentificacion implements OnInit {
  private readonly recurso = 'tipos-identificacion' as const;
  ...
  tipoDetalle: TipoIdentificacionModel | null = null;
  ...
  get sinRegistros(): boolean {
    return this.listLoaded && !this.loadPending && !this.loadError && this.tipos.length === 0;
  }
  ...
  get totalPaginas(): number {
    return Math.max(1, Math.ceil(this.tiposFiltrados.length / this.itemsPorPagina));
  }

  get tiposFiltradosPaginados(): TipoIdentificacionModel[] {
    const all = this.tiposFiltrados;
    const start = (this.paginaActual - 1) * this.itemsPorPagina;
    return all.slice(start, start + this.itemsPorPagina);
  }

  get hayMultiplesPaginas(): boolean {
    return this.tiposFiltrados.length > this.itemsPorPagina;
  }

  get mostrarBarraPaginacion(): boolean {
    return !this.sinResultadosPorFiltro && this.tiposFiltrados.length > 0;
  }
  ...
  private sincronizarPaginaConTotal(): void {
    const t = Math.max(1, Math.ceil(this.tiposFiltrados.length / this.itemsPorPagina));
  ...
  cargar(): void {
    ...
    this.clientesApi
      ...
      .subscribe({
        next: (data) => {
          this.tipos = Array.isArray(data) ? data : [];
        },
        error: (err) => {
  ...
  abrirNuevo(): void {
    ...
    setTimeout(() => this.showModal('modalTipoIdentificacionForm'), 0);
  }

  abrirEditar(tipo: TipoIdentificacionModel): void {
  ...
  abrirDetalle(tipo: TipoIdentificacionModel): void {
    this.tipoDetalle = tipo;
    this.saveError = '';
    setTimeout(() => this.showModal('modalTipoIdentificacionDetalle'), 0);
  }

  cerrarModalFormulario(): void {
    this.saveError = '';
    this.hideModal('modalTipoIdentificacionForm');
  }

  cerrarModalDetalle(): void {
    this.hideModal('modalTipoIdentificacionDetalle');
  ...
  guardar(): void {
    ...
    const vN = validarTipoIdentificacionNombreCreacion(this.form.nombre);
    if (!vN.valido) {
      this.saveError = vN.mensaje;
      return;
    }
    const vU = validarTipoIdentificacionNombreUnicoEnListado(this.tipos, this.form.nombre, null);
    if (!vU.valido) {
      this.saveError = vU.mensaje;
      return;
    }
    const vD = validarTipoIdentificacionDescripcionCreacion(this.form.descripcion);
    if (!vD.valido) {
  ...
  private guardarEdicion(): void {
    ...
    const vNom = validarTipoIdentificacionNombreContenido(f.nombre);
    if (!vNom.valido) {
      this.saveError = vNom.mensaje;
      return;
    }
    const vUni = validarTipoIdentificacionNombreUnicoEnListado(this.tipos, f.nombre, id);
    if (!vUni.valido) {
      this.saveError = vUni.mensaje;
      return;
    }
    const vDesc = validarTipoIdentificacionDescripcionContenido(f.descripcion);
    if (!vDesc.valido) {
    ...
    this.clientesApi
      ...
      .subscribe({
        next: (saved) => {
          const ix = this.tipos.findIndex((t) => t.id === saved.id);
          if (ix >= 0) this.tipos[ix] = saved;
  ...
  activarDesactivar(tipo: TipoIdentificacionModel): void {
    this.actionError = '';
    if (this.accionId != null) return;
    this.accionId = tipo.id;
    this.cdr.detectChanges();
    this.clientesApi
      .catalogoUpdate(this.recurso, tipo.id, { estadoActivo: !tipo.estadoActivo })
      .pipe(
        finalize(() => {
          this.accionId = null;
          this.cdr.detectChanges();
      ...
      .subscribe({
        next: (updated) => {
          const ix = this.tipos.findIndex((t) => t.id === tipo.id);
          if (ix >= 0) this.tipos[ix] = updated;
          this.tipos = [...this.tipos];
        },
        error: (err) => {
          const msg = err?.error?.message;
